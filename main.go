...
func main() {
	...
	for _, item := range r.Items {
		...
		//
		//if item.PageId != "none" {
		//	err := json.Unmarshal([]byte(item.PageId), &pageId)
		//	if err != nil {
		//		log.Fatalln(err)
		//	}
		//}
		...
		if len(item.Localizations.EnUs.Microsite.Ui.Slideshow) == 1 {
			slideshow = item.Localizations.EnUs.Microsite.Ui.Slideshow[0]
		}

		if len(item.Localizations.EnUs.DisclaimerDeviceV2) == 1 {
			disclaimerDeviceV2 = item.Localizations.EnUs.DisclaimerDeviceV2[0]
		}

		if len(item.Compatibility) == 1 {
			compatibility = item.Compatibility[0]
		}

		if len(item.Localizations.EnUs.Microsite.Upsell) == 1 {
			upsell = item.Localizations.EnUs.Microsite.Upsell[0]
		}
		...
		offerDetail := OfferDetail{
			...
			//CreatedBy:                         _PLACEHOLDER_STRING_, // Created by Offersvs API
			//CreatedTimestamp:                  _PLACEHOLDER_TIME_,   // Created by Offersvs API
			//LastModifiedBy:                    _PLACEHOLDER_STRING_, // Created by Offersvs API
			//LastModifiedTimestamp:             _PLACEHOLDER_TIME_,   // Created by Offersvs API
		}
